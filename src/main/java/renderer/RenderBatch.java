...
public class RenderBatch implements Comparable<RenderBatch> {
    ...
    public void addSprite(SpriteRenderer spr) {
        ...
        int index = this.numSprites; // index = where we want to put it- at the end of the array
    ...
    private void loadVertexProperties(int index) {
        // Need to create 4 vertices per quad
        ...
        // *    *
        // *    *
        // ^ We position everything based on the bottom left corner
        // So we control it by saying add the top right (1.0, 1.0)
        // then we add the bottom right (1.0, 0.0)
        float xAdd = 1.0f;
        float yAdd = 1.0f;
        for (int i=0; i < 4; i++) {
            if (i == 1) {
            ...
            Vector4f curPos = new Vector4f(sprite.gameObject.transform.position.x + (xAdd * sprite.gameObject.transform.scale.x),
                                            sprite.gameObject.transform.position.y + (yAdd * sprite.gameObject.transform.scale.y),
                                            0, 1);
            if (isRotated) {
                curPos = new Vector4f(xAdd, yAdd, 0, 1).mul(transformMatrix);
            }

            // Load position
            vertices[offset] = curPos.x;
            vertices[offset + 1] = curPos.y;
    ...
    private void loadElementIndices(int[] elements, int index) {
        ...
        elements[offsetArrayIndex + 2] = offset; // offset + 0
